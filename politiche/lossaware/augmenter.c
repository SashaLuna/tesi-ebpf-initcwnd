/* augmenter.c - politica «lossaware»: media mobile con penalizzazione sulle
 * perdite.
 *
 * Come «ewma», con una regola in più: le connessioni che hanno ritrasmesso
 * non entrano nella media e fanno dimezzare la stima. Le ritrasmissioni sono
 * il solo segnale che dice «la finestra iniziale è troppo grande» senza
 * dover modellare coda e capacità del percorso.
 *
 * Esempio:
 *   ./augmenter --iw-min 4 --iw-max 469 --alpha-shift 2 --retrans-thresh 0 \
 *               --csv server_lossaware_bw100_200K_it01.csv \
 *               --policy-label lossaware \
 *               --run-id lossaware_bw100_200K_it01 \
...
static struct {
	...
	unsigned int  retrans_thresh;/* oltre questo numero di ritrasmissioni si penalizza */
	unsigned int  crescita_max;  /* discriminante, in % di iw_applied; 0 = spento */
} cfg = {
	.cgroup       = "/sys/fs/cgroup",
	.csv_path     = NULL,
	.run_id       = "run",
	.policy_label = "lossaware",
	.bw_mbit      = 0,
	.obj_bytes    = 0,
	.iteration    = 0,
	...
	.retrans_thresh = 0,         /* qualunque ritrasmissione conta */
	/* Spento di default: 150 è il valore suggerito dalle misure. */
	.crescita_max = 0,
};
...
static void parse_args(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		/* opzioni della sola politica */
		else if (!strcmp(a, "--retrans-thresh") && has_val) cfg.retrans_thresh = atoi(argv[++i]);
		else if (!strcmp(a, "--crescita-max")   && has_val) cfg.crescita_max = atoi(argv[++i]);
		else
...
struct dst_state {
	...
	__u32 penalties;   /* quante volte è scattato il dimezzamento */
...
static unsigned int politica(struct dst_state *d, const struct aug_stats *st)
{
	...
	/* Sopra soglia il campione non entra nella media e la stima si dimezza.
	 * Si dimezza lo stato, non la sola uscita: altrimenti il campione buono
	 * successivo rileggerebbe la media intatta e la finestra tornerebbe
	 * subito dov'era. Il campione che ha perso si butta invece di pesarlo
	 * meno: è il punto in cui la coda è traboccata, non la capacità. */
	if (st->retrans > cfg.retrans_thresh) {

		/* Discriminante, opzionale. Non tutte le perdite sono colpa della
		 * finestra iniziale: se max_cwnd è vicina a iw_applied la raffica
		 * iniziale era troppo grande, se è molto più alta ha perso lo slow
		 * start salendo e penalizzare la finestra sarebbe un errore. La soglia
		 * è in percentuale di iw_applied; le misure che portano a 150 sono
		 * nel capitolo 6. */
		if (cfg.crescita_max && st->iw &&
		    (unsigned long long)st->cwnd * 100 >
		    (unsigned long long)st->iw * cfg.crescita_max) {
			d->scartati++;
			return 0;
		}
		...
		/* Alla prima penalizzazione la stima può essere ancora vuota:
		 * dimezzare zero darebbe zero, quindi si parte dalla finestra
		 * effettivamente usata. */
		if (d->ewma_fx == 0)
			d->ewma_fx = (st->iw ? st->iw : cfg.iw_min) << FRAC_BITS;

		d->ewma_fx >>= 1;

		/* Il pavimento è iw_min: zero è la sentinella «non intervenire»
		 * del programma nel kernel, e la politica sparirebbe in silenzio. */
		if (stima(d) < cfg.iw_min)
			d->ewma_fx = cfg.iw_min << FRAC_BITS;
...
static int run_analyzer(void *ctx, void *data, size_t len)
{
	...
	printf("analyzer: dst=%s IW %u->%u max_cwnd=%u campione=%u "
	       ...
	       iw ? iw : d->iw_current,
	       st->retrans > cfg.retrans_thresh
	       ? (iw ? "  (PENALIZZATA: dimezzata)"
		     : "  (perdita non attribuita alla finestra iniziale)")
	...
	if (csv) {
		fprintf(csv,
			 ...
			 * penalties conta i dimezzamenti verso quella destinazione. */
			stima(d), d->samples, d->penalties);
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr,
		...
		"           dimezzamento sopra %u ritrasmissioni, discriminante %s\n",
		cfg.policy_label, 1u << cfg.alpha_shift,
		cfg.iw_min, cfg.iw_max, cfg.min_bytes, cfg.retrans_thresh,
		cfg.crescita_max ? "acceso" : "spento");
	...
	for (i = 0; i < MAX_DSTS; i++) {
		...
		inet_ntop(AF_INET, &a, ip, sizeof(ip));
		fprintf(stderr,
			"    %-15s stima=%-5u campioni=%-4u scartati=%-4u dimezzamenti=%u%s\n",
